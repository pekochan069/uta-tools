 ...
 */
export async function getVips(): Promise<typeof Vips> {
  if (vipsInstance) {
    return vipsInstance;
  }
...
export async function resizeImage(
  ...
  if (!resize.crop) {
    ...
    if (width < pageHeight) {
      resizeWidth =
        resize.width >= resize.height ? resize.width : (width / pageHeight) * resize.height;
      thumbnailOptions.height =
        resize.width >= resize.height ? (pageHeight / width) * resizeWidth : resize.height;
    } else {
      resizeWidth =
        resize.width >= resize.height ? (width / pageHeight) * resize.height : resize.width;
      thumbnailOptions.height =
        resize.width >= resize.height ? resize.height : (pageHeight / width) * resizeWidth;
    }
 ...
 */
export async function hasTransparency(buffer: ArrayBuffer): Promise<boolean> {
  const vips = await getVips();
